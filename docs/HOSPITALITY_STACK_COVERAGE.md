...
|---|---|---|---|
| Local MQTT control | MQTT, local broker, cloud broker, retained messages, QoS 1 expectations | Cover now / cover next | MQTT retained-state behavior and minimal MQTT 3.1.1 serve ingress exist today. Standard-client verification, QoS1/reconnect/session boundaries belong to the protocol track. |
...
| Control panel / electrical | 24V power, UPS, redundant PSU, circuit protector, edge computer redundancy, industrial switches | Cover next / mock | These should become QA fault profiles with BMS evidence. `roomci` must not claim electrical safety validation. |
...
| Access / intercom | DoorBird, ONVIF, SIP, DTMF, Twilio SIP Trunk, UniFi Door Access, Okta/LDAP, electric lock panels | Mock / contract only | Access drift is safe QA coverage. Intercom and access actions should remain contract/mocked events. Real unlock authorization is out of scope. |
...
| Broader standards | BACnet, OPC UA, Zigbee, Thread, Matter, KNX | Future profile | Track official references and candidate subsets. Do not imply implementation until a narrow endpoint exists. |
