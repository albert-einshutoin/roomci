...
|---|---|---|---|---|---|
| MQTT 3.1.1 | OASIS MQTT Specification, https://mqtt.org/mqtt-specification/ | `conformance_subset`, `serve_endpoint`, `external_client_tested` | `CONNECT`, `CONNACK`, QoS0 `PUBLISH`, UTF-8 topic names, JSON object payloads, configured command/state topic mapping, retained-state observation through HTTP reports | `cargo test -p roomci-serve --lib external_mqtt`; `cargo test -p roomci-cli --test cli external_mqtt_publish_updates_retained_state_through_serve`; `make protocol-smoke-mqtt` | QoS1/QoS2 wire acks, `SUBSCRIBE`, retained replay to MQTT subscribers, sessions, will messages, keepalive enforcement, auth/TLS, clustering, persistence |
