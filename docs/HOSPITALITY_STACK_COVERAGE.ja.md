...
| Network / WAN | UniFi, YAMAHA, Starlink, VLAN, VPN, firewall, L3 switch, Wi-Fi roaming | Cover now | WAN outage and backup-link behavior exist. VLAN segment isolation, firewall drift, and local broker unreachable profiles are deterministic QA fault evidence, not vendor device emulators. |
| Control panel / electrical | 24V power, UPS, redundant PSU, circuit protector, edge computer redundancy, industrial switches | Cover now / mock | UPS degradation, circuit-protector trip, redundant PSU degradation, and secondary-edge takeover failure are deterministic QA fault profiles with BMS evidence. `roomci` must not claim electrical safety validation. |
| Comfort sensors / HVAC | temperature, humidity, occupancy-zone sensors, ceiling-zone sensors, discomfort index, HVAC auto mode | Cover now | Comfort automation and deterministic sensor-zone time-series replay exist, including discomfort-index final-state evidence. Hardware and thermal simulation remain out of scope. |
...
| 設計 / construction | Rebro, CAD, UniFi 設計 Center, BuddyBoard, draw.io, KiCad, JLCPCB, Blender, 3D printing | Out of scope | These are design/construction tools, not local/CI QA emulator surfaces. |
