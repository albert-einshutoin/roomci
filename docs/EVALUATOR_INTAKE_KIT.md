...
| Modbus/register map | Unit id, address, register type, data representation, scale/unit, read/write access, valid range, expected commissioning value | `modbus.devices[].id`, `unit_id`, `registers[].address`, `name`, `type`, `access`, `scale`, `unit`; customer-specific legal ranges have no contract field and remain review-only/blocked | `modbus.devices[]`, `steps[].modbus_write`, Modbus assertions; `make poc-building-automation` proves the public baseline, not customer-specific range enforcement |
| BMS alert schema | Source id, schema/content type, severity vocabulary, signed headers, replay window, channels, ticket states, redacted accepted/rejected samples | `bms.alerts[].id`, `source`, `schema_version`, `content_type`, `severity`, `severity_enum`, `hmac`, `replay_window_seconds`, `channels`, `ticket_lifecycle` record metadata | `alerts`, `contacts`, BMS/contact steps and assertions; `make poc-bms-ops` proves only the internal public baseline. Adapter HMAC/replay metadata is not applied to the external webhook, so customer signed-header, replay-window, and payload acceptance evidence remains review-only/blocked |
...
- [ ] Unsupported function codes and out-of-range values have expected failure
  behavior. Customer-specific legal ranges remain review-only/blocked because
  the current adapter and scenario contracts cannot encode or enforce them.
...
- [ ] Signature header/algorithm and replay window use non-production secret
  references only. These adapter fields are metadata and are not enforced by
  the current external webhook.
