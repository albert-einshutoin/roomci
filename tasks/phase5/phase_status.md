...
| `02_compose_ci_task.md` | `done` | Codex | `docker compose -f compose/docker-compose.yml up --build --abort-on-container-exit --exit-code-from scenario-smoke`; GitHub Actions command parity review | Compose and CI build local image and use supported CLI flags |
...
| Compose smoke test | `done` | `docker compose -f compose/docker-compose.yml up --build --abort-on-container-exit --exit-code-from scenario-smoke` |
