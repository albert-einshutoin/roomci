...
| Phase 14 | `done` | Hospitality smart-home QA core coverage: local MQTT, edge, device protocols, network/control faults, BMS/ops, comfort, and safe access/intercom boundaries | Private hospitality-stack compatibility and full-stack emulation remain out of scope | `phase14/phase_status.md` |
...
| Phase 26 | `in_progress` | Adapter contract generalization: typed payload and topic/identity diagnostics are complete; acceptance/assertion mapping is next | None | `phase26/phase_status.md` |
...
- **Phase 23** cleaned up public OSS trust issues found in product review:
...
- **Phase 24** completed deeper engineering hardening: typed assertion
  classification, runtime domain sub-states, bounded parser tests, concurrent
  serve overlay tests, and a protocol-server strategy ADR.
