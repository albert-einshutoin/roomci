...
| `02_intercom_commissioning_task.md` | `done` | Codex | `cargo test`; `examples/access_permission_drift.yaml` runs; `examples/commissioning_checklist.yaml` runs; `future_milestones.md` | Access-control drift and commissioning checklist are executable; deeper intercom work remains post-MVP |
...
| Access-control drift tests | `done` | `cargo run -p roomci-cli -- run examples/access_permission_drift.yaml` |
| Commissioning checklist tests | `done` | `cargo run -p roomci-cli -- run examples/commissioning_checklist.yaml` |
